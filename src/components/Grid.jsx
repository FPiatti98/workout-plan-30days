...
export default function Grid() {
    ...
    useEffect(()=>{
        ...
        if(localStorage.getItem('brogram')) {
            saveData = JSON.parse(localStorage.getItem('brogram'))
        }
        ...
        <div className="training-plan-grid">
            {Object.keys(training_plan).map((workout, workoutIndex) => {
                ...
                if(workoutIndex === selectedWorkout) {
                    return(
                        <WorkoutCard key={workoutIndex} trainingPlan={trainingPlan} workoutIndex={workoutIndex} type={type} icon={icon} dayNum={dayNum} handleComplete={handleComplete} handleSave={handleSave} savedWeights={savedWorkouts?.[workoutIndex]?.weights}/>
                    )
                }
                return (
                    <button onClick={() => {
